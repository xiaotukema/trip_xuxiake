...
- **点击后生成**：页面初始展示空状态，用户点击「开始生成 3 套行程方案」后才会产出结果，避免误以为是写死模板。
- **大模型优先**：高级设置里支持填写 `POST /api/ai/itineraries` 类大模型规划接口；接口不可用或未配置时，页面会明确提示并切换到本地智能预览，而不是假装拿到了实时数据。
...
随后打开 <http://localhost:4173>。在 GitHub Codespaces 中请打开 Ports 面板里的 `4173` 端口；页面加载后选择偏好并点击「开始生成 3 套行程方案」。
...
前端会向高级设置中填写的 `aiEndpoint` 发送 `POST` 请求。生产环境建议在服务端完成 LLM 调用、授权数据检索、价格聚合和风控审核，避免在浏览器暴露模型密钥。
...
```json
{
  ...
  "inspiration": "不想太赶，想看海和日落，酒店要方便打车",
